...
class ScannerTest extends munit.FunSuite{
  ...
  test("Scan for punctuators") {
    ...
    assertEquals(iterator.next(), DollarToken(createSourcePosition(1, 4)))
  ...
  test("Scan for keywords") {
    ...
    assertEquals(iterator.next(), ClusterToken(createSourcePosition(1, 34)))
    assertEquals(iterator.next(), RepToken(createSourcePosition(1, 42)))
